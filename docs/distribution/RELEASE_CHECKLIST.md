...
- Verify `aapt2 dump permissions app/build/outputs/apk/debug/app-debug.apk` contains no network, location, contacts, broad media, screenshot, or overlay-management permission.
...
- Test selected target app trigger, non-target app no-trigger, Settings/permission/dialer exclusion, skip once, end focus, and delete data including persisted file read-grant cleanup.
